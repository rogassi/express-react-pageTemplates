    ...
    this.render(viewPath, context, options)
        .then(function (body) {
            ...
            if (layoutPath) {
                ...
                var _body = this._getFile(layoutPath, options).then(function (str) {
                    var _template = context.template;
                    var templateOut = [];
                    var last = 0;
                    //var rex = /<REACT.*"(.*)".*\/>/gm;
                    var rex = /<(REACT).*"(.*)".*\/>|<(SEO).*"(.*)".*\/>|<([title|TITLE]+).*>(.*)<\/.*[title|TITLE]>/gm;
                    var myArray;
                    var fileToEnd = null;
                    while ((myArray = rex.exec(str)) != null) {
                    ...
                    }else if (context.seo && myArray[3]=="SEO")
                    {
                        switch (myArray[4].toUpperCase()) {
                            case "TITLE":
                                if(context.seo.title)
                                    component = "<title>" + context.seo.title + "</title>";
                                break;
                            case "DESCRIPTION":
                                if(context.seo.description)
                                    component = "<meta name=\"description\" content=\"" + context.seo.description + "\">";
                                break;
                            case "KEYWORDS":
                                if(context.seo.keywords)
                                    component = "<meta name=\"keywords\" content=\"" + context.seo.description + "\">";
                                break;
                        }
                    }
                    else if (context.seo && context.seo.title && myArray.length > 0 && myArray[5].toUpperCase() =="TITLE")
                    {
